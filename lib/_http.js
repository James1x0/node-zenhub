...
var API_URL = 'https://api.zenhub.io';
...
class Http {
    ...
    /**
     * Helper to handle PATCH requests to the API with authorization
     *
     * @private
     * @param string    url             address part after API root
     * @param object    parameters      additional parameters
     ...
     * @method          _put
     */
    _patch (url, parameters, body, isBeta) {
        if (!isBeta) {
            parameters = extend(parameters, this.credentials); // Add credentials to parameters
        }
        var patchURL = (isBeta ? BETA_API_URL : API_URL) + '/' + url + '?' + querystring.stringify(parameters); // Construct URL with parameters
